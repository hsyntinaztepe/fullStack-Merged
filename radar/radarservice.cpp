...
// Hareket mantığı 1 birim ötede aynı ID'ye sahip cisim oluşturuyor.. 
 ...
 
    {
        ...
        for (auto &kv : targets_)
        {
            ...
            if ((std::rand() % 100) < 20)
            {
                ...
                if ((std::rand() % 100) < 5)
                {
                    t.velocity += (std::rand() % 31 - 15); 
                    if (t.velocity < 0)
                        t.velocity = 0;

                    double factor = 1.0 + ((std::rand() % 2 == 0) ? 0.007 : -0.007); 
            ...
            else
            {
                ...
                if ((std::rand() % 100) < 10)
                {
                    double tiny_heading_change = (std::rand() % 5 - 2); 
